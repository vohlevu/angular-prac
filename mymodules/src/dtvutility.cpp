...
void UTCToDayTime(RMuint32 uiSecond, DayTime_dtv* pDT)
{
	...
	gmtime_r((time_t*) &uiSecond, &sTimeInfo);
...
{
	printf("%s: (DD:MM:YYYY, HH:MM:SS) = %2.2d:%2.2d:%4.4d, %2.2d:%2.2d:%2.2d \n",
		sComment,
		sDT.ymdw.day, sDT.ymdw.month, sDT.ymdw.year,
		sDT.hms.hour, sDT.hms.minute, sDT.hms.second);
...
void PrintTimeZone(char* sComment, TimeZone_dtv sTZ)
{
	printf("%s: (HH:MM), Status = (%2.2d:%2.2d), %d \n",
		sComment,
		sTZ.hour, sTZ.minute, sTZ.status);
...
RMint16 ParseYMDW(RMuint8 *p, struct YearMontDayWeek_dtv *pYmdw)
{
	...
	//	M1 &= 0xFFFF;
	//	printf("M1[%x]\n", M1);
	pYmdw->day = MJD - 14956 - (RMuint16)(Y1*365.25) - (RMuint16)(M1*30.6001);
	//	printf("Day[%x]\n", pYmdw->day);
	if ((M1 == 14) || (M1 == 15))
		K = 1;
	else
